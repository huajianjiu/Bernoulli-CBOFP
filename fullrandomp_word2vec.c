...
void *TrainModelThread(void *id) {
  ...
  unsigned long long next_random = (long long)id, next_next_random = (long long)id;
  int sample_reject = 0;
        ...
        if (negative_local > 0) for (d = 0; d < negative_local + 1 + PPDB_TABLE_SIZE; d++) {
          if (d == 0) {
            ...
            if (dropout == 1){
              ...
              next_random = next_random * (unsigned long long)25214903917 + 11;
              next_next_random = next_random * (unsigned long long)25214903917 + 11;
              if (next_random < next_next_random) fxik = 0;
              else fxik = 1;
              if (!fxik)
              {
                negative_local -= negative;
              ...
              for (wi = 0; wi < PPDB_TABLE_SIZE; wi++){
                if (paraphrases[word * PPDB_TABLE_SIZE + wi] > 0 && target == paraphrases[word * PPDB_TABLE_SIZE + wi]) {
                  ...
                  if (dropout == 1){
                    ...
                    next_random = next_random * (unsigned long long)25214903917 + 11;
                    next_next_random = next_random * (unsigned long long)25214903917 + 11;
                    if (next_random < next_next_random) sample_reject = 1;
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *)"-read-paraphrases", argc, argv)) > 0) {
    ...
    printf("Read External Paraphrases: %s\n", ppdb_file);
  } else {
